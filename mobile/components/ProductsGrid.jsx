...
export default function ProductsGrid({ products, isLoading }) {
    ...
    const renderProduct = ({ item: product }) => (
        <TouchableOpacity
            className="bg-[#161616] rounded-3xl overflow-hidden mb-3"
            style={{ width: "48%" }}
        ...
        >
            ...
            <View className="p-3">
                <Text className="text-gray-400 text-xs mb-1">{product.category}</Text>
            ...
            <View className="flex-row items-center justify-between p-3">
                <Text className="text-white font-bold text-lg">${product.price.toFixed(2)}</Text>
                <TouchableOpacity
                    className="bg-[#1DB954] rounded-full w-8 h-8 items-center justify-center"
                    activeOpacity={0.7}
...
function NoProductsFound() {
    return (
        <View className="py-20 items-center justify-center">
            ...
            <Text className="text-white font-semibold mt-4">No products found</Text>
            <Text className="text-gray-400 text-sm mt-2">Try adjusting your search or filter</Text>
        </View>
    )
}
